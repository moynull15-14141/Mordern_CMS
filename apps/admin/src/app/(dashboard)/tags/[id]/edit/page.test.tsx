...
vi.mock('@/features/tags/services/tags.api', () => ({ tagsApi: { get: vi.fn(), update: vi.fn() } }));
...
describe('EditTagPage', () => {
  it('renders the edit form for a user with category.create', async () => {
    vi.mocked(tagsApi.get).mockResolvedValue(tag);

    const element = await EditTagPage({ params: Promise.resolve({ id: 't1' }) });
