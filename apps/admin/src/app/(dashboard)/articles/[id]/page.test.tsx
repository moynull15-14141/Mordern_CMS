...
vi.mock('@/features/articles/services/articles.api', () => ({
  articlesApi: { get: vi.fn(), listRevisions: vi.fn() },
}));
...
describe('ArticleDetailPage', () => {
  it('renders article details for a user holding any relevant permission', async () => {
    vi.mocked(articlesApi.get).mockResolvedValue(article);
    vi.mocked(articlesApi.listRevisions).mockResolvedValue([]);

    const element = await ArticleDetailPage({ params: Promise.resolve({ id: 'a1' }) });
  ...
  it('redirects to /403 for a user with none of the article permissions', async () => {
