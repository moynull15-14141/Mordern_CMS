 ...
 * set only via the dedicated Publish/Schedule actions, never this form.
 * Same `body`/block-list substitution as `createArticleSchema` — see that
 * file's comment. No `authorId`/`language`/`locale` fields —
 * `UpdateArticleDto` doesn't expose them as editable. */
